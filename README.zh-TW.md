...
- Agent worktree 能讓每個 Agent 各自在隔離環境工作——但它只管「在哪裡做」，不管「誰做什麼、怎麼合作、誰把關」。你得到的是一堆獨立的產出，不是一份整合好的成果

**Claude Teams Go 的做法：**

一個視窗、一條指令，直接拿到結果。只用 **Hooks + Markdown**，就能讓多個 Agent 有角色、有分工、有流水線地協作——指揮官派任務、整合人把關、潤稿師跟你來回修到滿意。不需要安裝執行環境、不需要套件管理器、不需要學新語言。寫一份藍圖，跑 `/teams`（或 `/組隊`），你的團隊就會按照流程合作，交出一份完整的成果。
